...
def plot_spec(wave, flux, error, z, info, smooth=1):
    ...
    for wd in range(len(window_xrange)):
        if (wave.min()<window_xrange[wd][0]) and (wave.max()>window_xrange[wd][1]):
            ...
            axis.plot(wave[ind_wd], flux[ind_wd], c='k', ls='-', lw=0.8, alpha=0.8)
            for ll in range(len(line_cen)):
...
if __name__ == "__main__":
    ...
    input_catalog = '/data4/sdss5/master/spAll-master-qso_candidate.fits'
    ...
    if os.path.exists(input_catalog):
        ...
        # Cut the catalog to only QSO objects
        # See for details: https://sdss-wiki.atlassian.net/wiki/spaces/BHM/pages/43057229/Quasar+Visual+Inspection
        print('Selecting quasars from the catalog.\nSee for details: https://sdss-wiki.atlassian.net/wiki/spaces/BHM/pages/43057229/Quasar+Visual+Inspection.')
        select_qso = np.where(((vi_catalog.DR16Q==1))\
                  |((vi_catalog.CLASS=='STAR')&(vi_catalog.DR16Q==0))
                  |((vi_catalog.SN_MEDIAN_ALL<1)&(vi_catalog.DR16Q==0)&(np.char.find(vi_catalog.FIRSTCARTON, 'mwm')>=0))\
                  |((vi_catalog.CLASS=='GALAXY')&(vi_catalog.DR16Q==0))
                  , False, True)
        qso_index = np.arange(len(vi_catalog))[select_qso]
        print('Number of QSOs in the catalog: '+str(sum(select_qso))+'\n')
        ...
        if response.isdigit():
            if 0 < int(response) < sum(select_qso):
        ...
        for i in range(start_index, len(qso_index)):
            obj_line = ''
            ind_obj = qso_index[i]
